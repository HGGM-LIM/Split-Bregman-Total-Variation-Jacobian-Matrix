...
function [u,errAll] = TV_SB_2D(J,f, N,mu, lambda, gamma, nInner, nBreg,varargin)
% [u,errAll] = TV_SB_2D(J,f, N,mu, lambda, gamma, nInner,nBreg, uTarget)
% for a general Jacobian J.  
% [u,errAll] = TV_SB_2D(J,f, N,mu, lambda, gamma, nInner,nBreg, uTarget, R)
...
% Normalize Jacobian such that its Hessian diagonal is in the order of 1
normFactorJ = 1/sqrt(max(diag(J'*J)));
J           = J*normFactorJ;

% Scale the forward and adjoint operations so they don't depend on the
% image size 
scale       = 1/max(abs(J'*f));
...
for outer = 1:nBreg;    
    ...
    if nargin >= 9        
        ...
        if any([outer ==1, outer == 10, rem(outer, 100) == 0])
            close;
            h=figure;
            subplot(2,2,1);
            imagesc(abs(murf)); title(['retroprojection']); colorbar;
            subplot(2,2,2);
            imagesc(abs(u)); title(['u, iter. ' num2str(outer)]); colorbar;
            subplot(2,2,3);
